...
# 株価更新
def stock_make():
    embed_f = stock_module("data/stock_f.csv", "Asgaria Stock Exchange - Report", "アスガリア証券取引所:", "ファンタジア - アスガリア証券取引所", "output/stock_f.jpg")
    embed_n = stock_module("data/stock_n.csv", "New Saint City Stock Exchange - Report", "NCSE総合指数:", "ノクターン - NCSE総合指数", "output/stock_n.jpg")
...
def stock_module(data_path, emb_title, emb_description, graph_title, output_path):
    ...
    with open(data_path) as f:
        ...
        embed.add_field(name="```次回の経済情勢:```", value=tomorrow_economy, inline=True)
        embed.add_field(name="",
                          value="```―――――――🪙株価・物価情報―――――――```",
                          inline=False)
        embed.add_field(name="```現在株価:```",
                          value= str(today_stock) + "G",
                          inline=True)
